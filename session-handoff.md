...
Decide the regulariser question. Then: docs propagation across README, the report and the walkthrough,
which still carry "no graph variant reliably beats no-graph" as a finding rather than as a confounded
measurement. That sweep is large and deserves a session of its own.
