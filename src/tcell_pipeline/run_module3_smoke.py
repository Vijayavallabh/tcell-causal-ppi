...
def run() -> bool:
    required = (config.PROTEIN_EDGES_PATH, config.BLOCKED_SPLIT_PATH, config.PERTURBATION_CONDITION_PATH,
                config.DE_OBS_PATH, config.DE_VAR_PATH, zscore_path())
    ...
    with torch.no_grad():
        out = model(build_encoder_batch(pc, obs), targets, conditions)
    ...
    with torch.no_grad():
        out2 = expr_only(build_encoder_batch(pc, obs), targets, conditions)
