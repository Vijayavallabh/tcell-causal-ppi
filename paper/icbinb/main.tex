...
We report this as heterogeneity rather than as a positive result, because pooled it does not generalize: fixed-effect $+0.0139$ $[+0.0072,+0.0206]$ excludes zero, random-effects $+0.0326$ $[-0.0105,+0.0758]$ does not, at $I^2{=}88.2\%$ and $Q$ $p<0.001$. With heterogeneity that severe the random-effects reading is the honest one, and quoting the fixed-effect interval alone would be the most misleading single sentence available to us. What the result does establish is narrower and, for this venue, more useful: a graph-versus-no-graph null can be an artifact of the graph \emph{encoder} rather than evidence about the prior, and distinguishing the two costs one extra arm.
...
C. Architecture is wrong & $14$-cell inner-fold search over normalization, confidence pruning, edge-weighted convolution, attention; every lever neutral or worse & \textbf{Refuted} within the searched space \\
...
\paragraph{C. The architecture is not obviously wrong.} A $14$-cell search over per-relation normalization, edge-confidence pruning, STRING-weighted convolution and GATv2 attention \citep{velickovic2018gat,brody2022gatv2} leaves the default gated encoder on top, every lever neutral or worse (Appendix~\ref{app:archsearch}). Two are instructive: per-relation normalization depresses the mean gate by an order of magnitude ($0.03$ and $0.12$ against $0.73$), the \S\ref{sec:confound} failure arriving through another door, and pruning low-confidence functional edges lowers the score, which is evidence for E against the intuition that motivated it. Refuted within the searched space, which is the honest scope.
...
We set out to test whether a protein-interaction prior helps predict T-cell perturbation responses, and the harder problem was trusting our own measurement. A conventional regularizer switched off the mechanism under test while the pipeline kept reporting plausible numbers, in the direction we already expected. Repaired, the answer is a bounded null. Our one contrast that cleared multiplicity control then failed its own replication, and the fold statistic we would have used to explain that failure turned out to move as much under a reseed as across the range we were varying. What survives is smaller and more general: the baseline everyone calls ``no graph'' already carries graph-derived features, so this ablation does not measure what its name says, in anyone's hands.
