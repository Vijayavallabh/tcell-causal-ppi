...
RPE1 is not an isolated seed. Reading the untyped arm across every independent dataset we have, including the reference screen this paper is about, it is positive on five of five: reference $+0.0045$ (five of five seeds positive, $95\%$ CI $[+0.0011,+0.0079]$, Holm $0.042$ but Bonferroni $0.083$, so it fails our both-corrections rule), Replogle K562-essential $+0.0081$, Frangieh $+0.0194$, Tian $+0.0281$, Replogle RPE1 $+0.0675$. Individually all but RPE1 are underpowered. Pooled, random effects give $+0.0209$ ($95\%$ CI $[+0.0048,+0.0370]$, $p{=}0.011$; fixed-effect $+0.0056$ $[+0.0033,+0.0078]$) with $I^2{=}87.5\%$ and Cochran's $Q$ $p<0.001$. The sign test alone is $5/5$, $p{=}0.06$.

Two honesty conditions attach to that number and we state both. It is \textbf{post-hoc}: our pre-registration fixes a per-dataset contrast family, and a pooled meta-analytic test is not in it, so this is hypothesis-generating rather than confirmatory. And $I^2{=}88\%$ means the \emph{magnitude} does not transport -- the pooled estimate says a benefit exists somewhere in this family of datasets, not that you should expect $+0.02$ on yours.
...
That search was the wrong shape. Every one of its $14$ cells varied \emph{how} the typed, gated encoder does its message passing; not one removed the typing and gating themselves. The lever it never pulled is the one that moves: an untyped arm is positive on five of five independent datasets and beats the baseline outright on Replogle RPE1, where the typed arm is negative (\S\ref{sec:repl}). A search can only refute a cause inside the space it searches, and ours excluded its own premise. \textbf{Survives}, and on the evidence now available it is the cause that best explains the null.
