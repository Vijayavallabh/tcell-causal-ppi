...
The simulation in Appendix~\ref{app:power} says what this pipeline should be able to detect. This is
...
response reaches any injected value, including through the scaling constant, which is asserted by a test
that perturbs a validation response and requires the whole injection matrix to be bit-identical. And one
rung is a NEGATIVE CONTROL in which each target receives some \emph{other} target's neighbourhood, at the
...
\begin{table}[h]
\centering\small
\caption{Injected-signal ladder. $\Delta$ is \textsc{untyped\_gnn} $-$ \textsc{expression\_only},
paired over four seeds, corrected as one family of six with Bonferroni and Holm both required. The
last row is the negative control: the same injection magnitude over a scrambled neighbourhood.}
\label{tab:floor2}
\begin{tabular}{lrrrrl}
\toprule
condition & $\delta$ & $\Delta$ & $95\%$ CI & $p_{\mathrm{Bonf}}$ & \\
\midrule
no injection & $0$ & $+0.0048$ & $[-0.0000,+0.0097]$ & & zero point \\
injected & $0.02$ & $+0.0088$ & $[+0.0071,+0.0105]$ & $0.003$ & recovered \\
injected & $0.05$ & $+0.0081$ & $[+0.0052,+0.0110]$ & $0.018$ & recovered \\
injected & $0.10$ & $+0.0041$ & $[+0.0026,+0.0056]$ & $0.020$ & recovered \\
injected & $0.20$ & $+0.0090$ & $[+0.0072,+0.0109]$ & $0.003$ & recovered \\
injected & $0.40$ & $+0.0183$ & $[+0.0112,+0.0254]$ & $0.023$ & recovered \\
\textbf{scrambled} & $0.40$ & $\mathbf{-0.0001}$ & $[-0.0021,+0.0018]$ & $1.000$ & \textbf{not recovered} \\
\bottomrule
\end{tabular}
\end{table}

The floor is at or below $0.02$ response standard deviations, the smallest size we injected: every
injected rung is recovered under both corrections. The control is what makes that readable. At the same
magnitude, a scrambled neighbourhood produces a gap of $-0.0001$ with a confidence interval of width
$0.004$ around zero, so the gaps above are the graph being read and not the injection being large. The
absolute scores make the same point from the other side: at $\delta{=}0.40$ both arms gain $0.08$ to
$0.10$ \textsc{systema}, while under the scrambled injection of identical magnitude both arms
\emph{lose} a little. Injected structure that follows the graph makes the task easier for everything;
injected structure that does not follow it makes the task slightly harder for everything.

One caution belongs with the number, and we computed it because the primary comparison could not settle
it. Each rung is tested against zero, and this fold already carries a real graph benefit of $+0.0048$
with no injection at all, so a rung can clear on the pre-existing benefit alone. Subtracting the same
seed's un-injected gap leaves what the injection bought: $+0.0040$ at $\delta{=}0.02$
($p{=}0.14$), $+0.0042$ at $0.20$ ($p{=}0.11$), and $+0.0135$ at $0.40$ ($p{=}0.03$). So the pipeline
detects graph-dependent structure at $0.02$, and at four seeds it cannot distinguish an \emph{added}
benefit from the one already present until $\delta{=}0.40$. That subtraction is post-hoc and we say so;
it was written and committed while two rungs were still training, which is the only reason it is worth
reading.

The scrambled control also carries a finding of its own. Its increment over the zero point is
$-0.0050$ ($[-0.0083,-0.0016]$): injecting graph-shaped structure that follows the \emph{wrong} graph
does not merely fail to help, it destroys the real benefit that was there. A prior that is confidently
wrong is worse than no prior, and that is measurable here rather than asserted.
